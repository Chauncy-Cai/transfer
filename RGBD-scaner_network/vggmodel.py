...
#LAYER FOUR 2-CONV3-512 + MAXPOOL (32*32*256=>16*16*512)
...
#LAYER FIVE 2-CONV3-512 + MAXPOOL (16*16*512=>8*8*512)
l51x1, l51x2 = cnn_level_share(l42x1, l42x2, [3,3,512,512], 9, maxpool=False)
l52x1, l52x2 = cnn_level_share(l51x1, l51x2, [3,3,512,512], 10, maxpool=True)

#LAYER SIX full-connected
flat1 = tf.reshape(l52x1,[-1,8*8*512])
flat2 = tf.reshape(l52x2,[-1,8*8*512])

l61x1, l61x2 = fully_connect_share(flat1,flat2,8*8*512,4096,11)

l62x1, l62x2 = fully_connect_share(l61x1,l61x2,4096,4096,12)
l63x1, l63x2 = fully_connect_share(l62x1,l62x2,4096,1000,11)
#------ soft max-----

#FINAL
#normalize
y1 = tf.nn.l2_normalize(l63x1,dim=1)##?all?
y2 = tf.nn.l2_normalize(l63x2,dim=1)##?all? [?,30]
#loss
